...
export default function RulesPanel() {
  ...
  return (
    <div className="rounded-xl border border-neutral-100 bg-white p-4 shadow-[0_2px_12px_rgba(15,23,42,0.06)]">
      <p className="mb-2 font-serif text-xs font-semibold uppercase tracking-wide text-neutral-400">Rules</p>
      <ul className="flex flex-col gap-3">
        {RULE_FIELD_DEFS.map((field) => (
          <li key={field.key} className="flex items-center justify-between gap-3">
            <div>
              <p className="font-serif text-sm font-medium italic text-neutral-900">{field.label}</p>
              <p className="font-serif text-xs not-italic text-neutral-500">{field.description}</p>
            </div>
            <button
